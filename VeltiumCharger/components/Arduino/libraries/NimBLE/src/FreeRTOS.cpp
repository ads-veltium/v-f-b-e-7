...
uint32_t FreeRTOS::Semaphore::wait(std::string owner) {
    Serial.printf(">> wait: Semaphore waiting: %s for %s", toString().c_str(), owner.c_str());
    ...
    Serial.printf("<< wait: Semaphore released: %s", toString().c_str());
    return m_value;
} // wait
...
bool FreeRTOS::Semaphore::timedWait(std::string owner, uint32_t timeoutMs) {
    ...
    Serial.printf("<< wait: Semaphore %s released: %d", toString().c_str(), ret);
    return ret;
} // wait
...
void FreeRTOS::Semaphore::give() {
    Serial.printf("Semaphore giving: %s", toString().c_str());
    m_owner = std::string("<N/A>");
...
bool FreeRTOS::Semaphore::take(std::string owner) {
    Serial.printf("Semaphore taking: %s for %s", toString().c_str(), owner.c_str());
    bool rc = false;
    if (m_usePthreads) {
        pthread_mutex_lock(&m_pthread_mutex);
    } else {
    ...
    m_owner = owner;
    if (rc) {
        Serial.printf("Semaphore taken:  %s", toString().c_str());
    } else {
        Serial.printf("Semaphore NOT taken:  %s", toString().c_str());
    }
    return rc;
} // Semaphore::take
