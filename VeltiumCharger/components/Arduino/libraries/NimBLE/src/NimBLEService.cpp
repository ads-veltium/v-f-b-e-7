...
void NimBLEService::dump() {
    //Serial.printf("Service: uuid:%s, handle: 0x%2x",m_uuid.toString().c_str(),m_handle);
...
bool NimBLEService::start() {
    //Serial.printf(">> start(): Starting service: %s", toString().c_str());
    int rc = 0;
    ...
    if (rc != 0) {
        //Serial.printf("ble_gatts_count_cfg failed, rc= %d, %s", rc, NimBLEUtils::returnCodeToString(rc));
        return false;
    }

    rc = ble_gatts_add_svcs((const ble_gatt_svc_def*)m_pSvcDef);
    if (rc != 0) {
        //Serial.printf("ble_gatts_add_svcs, rc= %d, %s", rc, NimBLEUtils::returnCodeToString(rc));
        return false;
...
NimBLECharacteristic* NimBLEService::createCharacteristic(const NimBLEUUID &uuid, uint32_t properties) {
    ...
    if (getCharacteristic(uuid) != nullptr) {
        //Serial.printf("<< Adding a duplicate characteristic with UUID: %s",std::string(uuid).c_str());
    }
