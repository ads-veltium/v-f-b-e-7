...
NimBLEService* NimBLEServer::createService(const NimBLEUUID &uuid, uint32_t numHandles, uint8_t inst_id) {
    //Serial.printf(">> createService - %s", uuid.toString().c_str());
    // TODO: add functionality to use inst_id for multiple services with same uuid
    (void)inst_id;
    // Check that a service with the supplied UUID does not already exist.
    if(getServiceByUUID(uuid) != nullptr) {
        //Serial.printf("Warning creating a duplicate service UUID: %s",std::string(uuid).c_str());
    }
...
void NimBLEServer::start() {
    if(m_gattsStarted) {
        //Serial.printf("Gatt server already started");
        return;
    }

    int rc = ble_gatts_start();
    if (rc != 0) {
        //Serial.printf("ble_gatts_start; rc=%d, %s", rc,NimBLEUtils::returnCodeToString(rc));
        abort();
    }
...
/*** Future use ***
  * TODO: implement service changed handling
    ...
    if(rc != 0) {
        //Serial.printf("ble_gatts_find_chr: rc=%d, %s", rc,
                            NimBLEUtils::returnCodeToString(rc));
        abort();
    }

    //Serial.printf("Service changed characterisic handle: %d", m_svcChgChrHdl);
*/
...
int NimBLEServer::disconnect(uint16_t connId, uint8_t reason) {
    ...
    if(rc != 0){
        //Serial.printf("ble_gap_terminate failed: rc=%d %s", rc,NimBLEUtils::returnCodeToString(rc));
    }
    ...
    switch(event->type) {
        ...
        case BLE_GAP_EVENT_DISCONNECT: {
            ...
            switch(event->disconnect.reason) {
                ...
                case BLE_HS_ENOTSYNCED:
                    NIMBLE_LOGC(LOG_TAG, "Disconnect - host reset, rc=%d \n", event->disconnect.reason);
        ...
        case BLE_GAP_EVENT_PASSKEY_ACTION: {
            ...
            if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
                ...
                //Serial.printf("BLE_SM_IOACT_DISP; ble_sm_inject_io result: %d", rc);

            } else if (event->passkey.params.action == BLE_SM_IOACT_NUMCMP) {
                Serial.printf("Passkey on device's display: %d\n", event->passkey.params.numcmp);
                pkey.action = event->passkey.params.action;
                // Compatibility only - Do not use, should be removed the in future
                if(NimBLEDevice::m_securityCallbacks != nullptr) {
                ...
                rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
                Serial.printf("BLE_SM_IOACT_NUMCMP; ble_sm_inject_io result: %d\n", rc);
            ...
            //TODO: Handle out of band pairing
                ...
                rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
                Serial.printf("BLE_SM_IOACT_OOB; ble_sm_inject_io result: %d\n", rc);
            //////////////////////////////////
                ...
                rc = ble_sm_inject_io(event->passkey.conn_handle, &pkey);
                Serial.printf("BLE_SM_IOACT_INPUT; ble_sm_inject_io result: %d\n", rc);
                            ...
                            uint16_t latency, uint16_t timeout)
{
    ...
    if(rc != 0) {
        //Serial.printf("Update params error: %d, %s", rc, NimBLEUtils::returnCodeToString(rc));
    }
}// updateConnParams
...
void NimBLEServerCallbacks::onPassKeyNotify(uint32_t pass_key){
    Serial.printf( "onPassKeyNotify: default: %d \n", pass_key);
}

bool NimBLEServerCallbacks::onSecurityRequest(){
    Serial.println( "onSecurityRequest: default: true");
    return true;
}
*/
