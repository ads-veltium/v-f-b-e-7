...
void NimBLEAdvertising::setScanFilter(bool scanRequestWhitelistOnly, bool connectWhitelistOnly) {
    //Serial.printf(">> setScanFilter: scanRequestWhitelistOnly: %d, connectWhitelistOnly: %d",scanRequestWhitelistOnly, connectWhitelistOnly);
    if (!scanRequestWhitelistOnly && !connectWhitelistOnly) {
        m_advParams.filter_policy = BLE_HCI_ADV_FILT_NONE;
        //Serial.printf("<< setScanFilter");
        return;
    }
    if (scanRequestWhitelistOnly && !connectWhitelistOnly) {
...
void NimBLEAdvertising::setAdvertisementData(NimBLEAdvertisementData& advertisementData) {
    //Serial.printf(">> setAdvertisementData");
    int rc = ble_gap_adv_set_data(
        (uint8_t*)advertisementData.getPayload().data(),
        advertisementData.getPayload().length());
    if (rc != 0) {
        //Serial.printf( "ble_gap_adv_set_data: %d %s",rc, NimBLEUtils::returnCodeToString(rc));
    }
...
void NimBLEAdvertising::setScanResponseData(NimBLEAdvertisementData& advertisementData) {
    //Serial.printf(">> setScanResponseData");
    int rc = ble_gap_adv_rsp_set_data(
        (uint8_t*)advertisementData.getPayload().data(),
        advertisementData.getPayload().length());
    if (rc != 0) {
        //Serial.printf( "ble_gap_adv_rsp_set_data: %d %s",rc,  NimBLEUtils::returnCodeToString(rc));
    }
...
bool NimBLEAdvertising::start(uint32_t duration, void (*advCompleteCB)(NimBLEAdvertising *pAdv)) {
    //Serial.printf(">> Advertising start: customAdvData: %d, customScanResponseData: %d",m_customAdvData, m_customScanResponseData);

    // If Host is not synced we cannot start advertising.
    if(!NimBLEDevice::m_synced) {
        //Serial.printf( "Host reset, wait for sync.");
        return false;
    }
    ...
    if(pServer != nullptr) {
        if(!pServer->m_gattsStarted){
            ...
            //Serial.printf( "Max connections reached - not advertising");
            return false;
        }
    }
#endif
    ...
    if(ble_gap_adv_active()) {
        //Serial.printf("Advertising already active");
        return false;
    }
    ...
    if (!m_customAdvData && !m_advDataSet) {
        ...
        if(m_scanResp && !m_customScanResponseData) {
            ...
            switch(rc) {
                ...
                case BLE_HS_EBUSY:
                    //Serial.printf( "Already advertising");
                    break;

                case BLE_HS_EMSGSIZE:
                    //Serial.printf( "Scan data too long");
                    break;

                default:
                    //Serial.printf( "Error setting scan response data; rc=%d, %s",rc, NimBLEUtils::returnCodeToString(rc));
                    break;
            }
        }
        ...
        if(rc == 0) {
            ...
            switch(rc) {
                ...
                case BLE_HS_EMSGSIZE:
                    //Serial.printf( "Advertisement data too long");
                    break;

                default:
                    //Serial.printf( "Error setting advertisement data; rc=%d, %s",rc, NimBLEUtils::returnCodeToString(rc));
                    break;
            }
        }
    ...
    switch(rc) {
        ...
        case BLE_HS_EINVAL:
             //Serial.printf( "Unable to advertise - Duration too long");
             break;

        case BLE_HS_EPREEMPTED:
             //Serial.printf( "Unable to advertise - busy");
             break;
        ...
        case BLE_HS_ENOTSYNCED:
             //Serial.printf( "Unable to advertise - Host Reset");
             break;

        default:
            //Serial.printf( "Error enabling advertising; rc=%d, %s",rc, NimBLEUtils::returnCodeToString(rc));
            break;
    }
    ...
    //Serial.printf("<< Advertising start");
    return true;
} // start
...
void NimBLEAdvertising::stop() {
    ...
    if (rc != 0 && rc != BLE_HS_EALREADY) {
        //Serial.printf( "ble_gap_adv_stop rc=%d %s",rc, NimBLEUtils::returnCodeToString(rc));
        return;
    }
...
void NimBLEAdvertisementData::addData(const std::string &data) {
    if ((m_payload.length() + data.length()) > BLE_HS_ADV_MAX_SZ) {
        //Serial.printf( "Advertisement data length exceded");
        return;
    }
                                          ...
                                          const std::vector<NimBLEUUID> &v_uuid)
{
    ...
    for(auto &it : v_uuid){
        if(it.bitSize() != size) {
            //Serial.printf( "Service UUID(%d) invalid", size);
            return;
        } else {
            switch(size) {
                case 16:
