...
bool NimBLEClient::connect(const NimBLEAddress &address, bool deleteAttibutes) {
    //Serial.printf(">> connect(%s)", address.toString().c_str());
    ...
    if(isConnected() || m_connEstablished || m_pTaskData != nullptr) {
        //Serial.printf("Client busy, connected to %s, id=%d",
    ...
    if(ble_gap_conn_find_by_addr(&peerAddr_t, NULL) == 0) {
        //Serial.printf("A connection to %s already exists",
                    //address.toString().c_str());
        return false;
    }

    if(address == NimBLEAddress("")) {
        //Serial.printf("Invalid peer address;(NULL)");
        return false;
    } else {
    ...
    do {
        ...
        switch (rc) {
            ...
            case BLE_HS_EDONE:
                ...
                //Serial.printf("Already connected to device; addr=%s",
            ...
            case BLE_HS_EALREADY:
                ...
                //Serial.printf("Already attempting to connect to %s - cancelling",
                            //std::string(m_peerAddress).c_str());
                ble_gap_conn_cancel();
                break;

            default:
                /*Serial.printf("Failed to connect to %s, rc=%d; %s",
                            std::string(m_peerAddress).c_str(),
    ...
    if(ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(m_connectTimeout + 1000)) == pdFALSE) {
        ...
        /*Serial.printf("Connection failed; status=%d %s",
                    taskData.rc,
...
int NimBLEClient::disconnect(uint8_t reason) {
    ...
    if(isConnected()) {
        ...
        if(ble_npl_callout_is_active(&m_dcTimer)) {
            //Serial.printf("Already disconnecting, timer started");
            return BLE_HS_EALREADY;
        }

        ble_gap_conn_desc desc;
        if(ble_gap_conn_find(m_conn_id, &desc) != 0){
            //Serial.printf("Connection ID not found");
            return BLE_HS_EALREADY;
        }
        ...
        if (rc != 0) {
            ...
            /*Serial.printf("ble_gap_terminate failed: rc=%d %s",
                        rc, NimBLEUtils::returnCodeToString(rc));*/
        }
    } else {
        //Serial.printf("Not connected to any peers");
    }
                            ...
                            uint16_t latency, uint16_t timeout)
{
    ...
    if(rc != 0) {
        /*Serial.printf("Update params error: %d, %s",
                    rc, NimBLEUtils::returnCodeToString(rc));*/
    }
} // updateConnParams
...
void NimBLEClient::setPeerAddress(const NimBLEAddress &address) {
    if(isConnected()) {
        //Serial.printf("Cannot set peer address while connected");
        return;
    }

    m_peerAddress = address;
    //Serial.printf("Peer address set: %s", std::string(m_peerAddress).c_str());
} // setPeerAddress
...
int NimBLEClient::getRssi() {
    //Serial.printf(">> getRssi()");
    if (!isConnected()) {
        //Serial.printf("<< getRssi(): Not connected");
        return 0;
    }
    ...
    if(rc != 0) {
        /*Serial.printf("Failed to read RSSI error code: %d, %s",
                                rc, NimBLEUtils::returnCodeToString(rc));*/
        return 0;
    }
...
std::vector<NimBLERemoteService*>* NimBLEClient::getServices(bool refresh) {
    if(refresh) {
        ...
        if (!retrieveServices()) {
            //Serial.printf("Error: Failed to get services");
        }
        else{
            //Serial.printf("Found %d services", m_servicesVector.size());
        }
    }
 ...
 */
    ...
    //Serial.printf(">> retrieveServices");

    if(!isConnected()){
        //Serial.printf("Disconnected, could not retrieve services -aborting");
        return false;
    }
    ...
    if (rc != 0) {
        //Serial.printf("ble_gattc_disc_all_svcs: rc=%d %s", rc, NimBLEUtils::returnCodeToString(rc));
        return false;
    }
    ...
    if(taskData.rc == 0){
        ...
        //Serial.printf("Could not retrieve services");
        return false;
    }
} // getServices
                ...
                const struct ble_gatt_svc *service, void *arg)
{
    ...
    if(error->status == BLE_HS_EDONE) {
        ...
        /*Serial.printf("characteristicDiscCB() rc=%d %s",
                             error->status,
...
std::string NimBLEClient::getValue(const NimBLEUUID &serviceUUID, const NimBLEUUID &characteristicUUID) {
    /*Serial.printf(">> getValue: serviceUUID: %s, characteristicUUID: %s",
...
{
    //Serial.printf(">> setValue: serviceUUID: %s, characteristicUUID: %s",serviceUUID.toString().c_str(), characteristicUUID.toString().c_str());
 ...
 */
 ...
 int NimBLEClient::handleGapEvent(struct ble_gap_event *event, void *arg) {
    ...
    Serial.printf("Got Client event %s", NimBLEUtils::gapEventToString(event->type));
    ...
    switch(event->type) {
        ...
        case BLE_GAP_EVENT_DISCONNECT: {
            ...
            //Serial.printf("disconnect; reason=%d, %s",rc, NimBLEUtils::returnCodeToString(rc));
        ...
        case BLE_GAP_EVENT_CONNECT: {
            ...
            if (rc == 0) {
                //Serial.printf("Connected event");

                client->m_conn_id = event->connect.conn_handle;

                rc = ble_gattc_exchange_mtu(client->m_conn_id, NULL,NULL);
                if(rc != 0) {
                    //Serial.printf("MTU exchange error; rc=%d %s",rc, NimBLEUtils::returnCodeToString(rc));
                    break;
                }
        ...
        case BLE_GAP_EVENT_NOTIFY_RX: {
            ...
            //Serial.printf("Notify Recieved for handle: %d",event->notify_rx.attr_handle);
            ...
            for(auto &it: client->m_servicesVector) {
                ...
                /*Serial.printf("checking service %s for handle: %d",
                ...
                if(characteristic != cVector->cend()) {
                    //Serial.printf("Got Notification for characteristic %s"(*characteristic)->toString().c_str());
                    ...
                    if ((*characteristic)->m_notifyCallback != nullptr) {
                        //Serial.printf("Invoking callback for notification on characteristic %s",(*characteristic)->toString().c_str());
        ...
        case BLE_GAP_EVENT_L2CAP_UPDATE_REQ: {
            ...
            //Serial.printf("%s peer params", (rc == 0) ? "Accepted" : "Rejected");
            return rc;
        ...
        case BLE_GAP_EVENT_CONN_UPDATE: {
            ...
            if(event->conn_update.status == 0) {
                ...
                //Serial.printf("Update connection parameters failed.");
            }
            return 0;
        ...
        case BLE_GAP_EVENT_PASSKEY_ACTION: {
            ...
            if (event->passkey.params.action == BLE_SM_IOACT_DISP) {
                ...
                //Serial.printf("ble_sm_inject_io result: %d", rc);

            } else if (event->passkey.params.action == BLE_SM_IOACT_NUMCMP) {
                //Serial.printf("Passkey on device's display: %d", event->passkey.params.numcmp);
                pkey.action = event->passkey.params.action;
            ...
            ////////
                ...
                //Serial.printf("No passkey action required");
            }
