...
char* NimBLEUtils::buildHexData(uint8_t* target, const uint8_t* source, uint8_t length) {
    ...
    if (target == nullptr) {
        ...
        if (target == nullptr) {
            //Serial.printf("buildHexData: malloc failed");
            return nullptr;
        }
    }
...
void NimBLEUtils::dumpGapEvent(ble_gap_event *event, void *arg){
    ...
    //Serial.printf("Received a GAP event: %s", gapEventToString(event->type));
#endif
}
...
#endif
        default:
            //Serial.printf("gapEventToString: Unknown event type %d 0x%.2x", eventType, eventType);
