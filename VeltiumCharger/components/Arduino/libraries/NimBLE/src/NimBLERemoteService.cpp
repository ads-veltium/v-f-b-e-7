...
NimBLERemoteService::NimBLERemoteService(NimBLEClient* pClient, const struct ble_gatt_svc* service) {
    ...
    //Serial.printf(">> NimBLERemoteService()");
    m_pClient = pClient;
    ...
    //Serial.printf("<< NimBLERemoteService(): %s", m_uuid.toString().c_str());
}
...
NimBLERemoteCharacteristic* NimBLERemoteService::getCharacteristic(const NimBLEUUID &uuid) {
    //Serial.printf(">> getCharacteristic: uuid: %s", uuid.toString().c_str());

    for(auto &it: m_characteristicVector) {
        if(it->getUUID() == uuid) {
            //Serial.printf("<< getCharacteristic: found the characteristic with uuid: %s", uuid.toString().c_str());
            return it;
        }
    }
    ...
    //Serial.printf("<< getCharacteristic: not found");
    return nullptr;
} // getCharacteristic
...
std::vector<NimBLERemoteCharacteristic*>* NimBLERemoteService::getCharacteristics(bool refresh) {
    if(refresh) {
        ...
        if (!retrieveCharacteristics()) {
            //Serial.printf("Error: Failed to get characteristics");
        }
        else{
            //Serial.printf("Found %d characteristics", m_characteristicVector.size());
        }
    }
                                ...
                                const struct ble_gatt_chr *chr, void *arg)
{
    ...
    if(error->status == BLE_HS_EDONE) {
        ...
        //Serial.printf("characteristicDiscCB() rc=%d %s",error->status,NimBLEUtils::returnCodeToString(error->status));
...
bool NimBLERemoteService::retrieveCharacteristics(const NimBLEUUID *uuid_filter) {
    //Serial.printf(">> retrieveCharacteristics() for service: %s", getUUID().toString().c_str());
    ...
    if(taskData.rc == 0){
        //Serial.printf("<< retrieveCharacteristics()");
        return true;
    }

    //Serial.printf("Could not retrieve characteristics");
    return false;
...
std::string NimBLERemoteService::getValue(const NimBLEUUID &characteristicUuid) {
    //Serial.printf(">> readValue: uuid: %s", characteristicUuid.toString().c_str());
...
bool NimBLERemoteService::setValue(const NimBLEUUID &characteristicUuid, const std::string &value) {
    //Serial.printf(">> setValue: uuid: %s", characteristicUuid.toString().c_str());
...
void NimBLERemoteService::deleteCharacteristics() {
    //Serial.printf(">> deleteCharacteristics");
    for(auto &it: m_characteristicVector) {
        delete it;
    }
...
size_t NimBLERemoteService::deleteCharacteristic(const NimBLEUUID &uuid) {
    //Serial.printf(">> deleteCharacteristic");
