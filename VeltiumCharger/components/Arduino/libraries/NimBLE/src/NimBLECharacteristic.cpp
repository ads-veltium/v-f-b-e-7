                             ...
                             void *arg)
{
    ...
    //Serial.printf("Characteristic %s %s event", pCharacteristic->getUUID().toString().c_str(),ctxt->op == BLE_GATT_ACCESS_OP_READ_CHR ? "Read" : "Write");
...
void NimBLECharacteristic::setSubscribe(struct ble_gap_event *event) {
    ...
    //Serial.printf("New subscribe value for conn: %d val: %d",event->subscribe.conn_handle, subVal);
...
void NimBLECharacteristic::indicate() {
    //Serial.printf(">> indicate: length: %d", getDataLength());
    notify(false);
    //Serial.printf("<< indicate");
} // indicate
...
void NimBLECharacteristic::notify(bool is_notification) {
    //Serial.printf(">> notify: length: %d", getDataLength());
    ...
    if (m_subscribedVec.size() == 0) {
        //Serial.printf("<< notify: No clients subscribed.");
        return;
    }
    ...
    for (auto &it : m_subscribedVec) {
        ...
        if (length > _mtu - 3) {
            //Serial.printf("- Truncating to %d bytes (maximum notify size)", _mtu - 3);
        }
...
void NimBLECharacteristic::setValue(const uint8_t* data, size_t length) {
    ...
    //Serial.printf(">> setValue: length=%d, data=%s, characteristic UUID=%s", length, pHex, getUUID().toString().c_str());
    free(pHex);
#endif

    if (length > BLE_ATT_ATTR_MAX_LEN) {
        //Serial.printf("Size %d too large, must be no bigger than %d", length, BLE_ATT_ATTR_MAX_LEN);
        return;
    }
