...
void NimBLEEddystoneURL::setData(const std::string &data) {
    if (data.length() > sizeof(m_eddystoneData)) {
        //Serial.printf("Unable to set the data ... length passed in was %d and max expected %d",data.length(), sizeof(m_eddystoneData));
        return;
    }
...
void NimBLEEddystoneURL::setURL(const std::string &url) {
  if (url.length() > sizeof(m_eddystoneData.url)) {
    //Serial.printf("Unable to set the url ... length passed in was %d and max expected %d",url.length(), sizeof(m_eddystoneData.url));
    return;
  }
  memset(m_eddystoneData.url, 0, sizeof(m_eddystoneData.url));
