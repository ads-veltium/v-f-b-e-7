    ...
    switch(event->type) {
        ...
        case BLE_GAP_EVENT_DISC: {
            if(pScan->m_ignoreResults) {
                //Serial.printf("Scan op in progress - ignoring results");
                return 0;
            }
            ...
            if(NimBLEDevice::isIgnored(advertisedAddress)) {
                //Serial.printf("Ignoring device: address: %s", advertisedAddress.toString().c_str());
                return 0;
            }
            ...
            if(advertisedDevice == nullptr && event->disc.event_type != BLE_HCI_ADV_RPT_EVTYPE_SCAN_RSP){
                ...
                //Serial.printf("New advertiser: %s", advertisedAddress.toString().c_str());
            } else if(advertisedDevice != nullptr) {
                //Serial.printf("Updated advertiser: %s", advertisedAddress.toString().c_str());
            } else {
        ...
        case BLE_GAP_EVENT_DISC_COMPLETE: {
            //Serial.printf("discovery complete; reason=%d",event->disc_complete.reason);
...
bool NimBLEScan::start(uint32_t duration, void (*scanCompleteCB)(NimBLEScanResults), bool is_continue) {
    //Serial.printf(">> start(duration=%d)", duration);
    ...
    switch(rc) {
        ...
        case BLE_HS_EBUSY:
            //Serial.printf("Unable to scan - connection in progress.");
            break;
        ...
        default:
            //Serial.printf("Error initiating GAP discovery procedure; rc=%d, %s",rc, NimBLEUtils::returnCodeToString(rc));
            break;
    }
...
NimBLEScanResults NimBLEScan::start(uint32_t duration, bool is_continue) {
    if(duration == 0) {
        //Serial.printf("Blocking scan called with duration = forever");
    }
...
bool NimBLEScan::stop() {
    ...
    if (rc != 0 && rc != BLE_HS_EALREADY) {
        //Serial.printf("Failed to cancel scan; rc=%d", rc);
        return false;
    }
...
void NimBLEScan::erase(const NimBLEAddress &address) {
    //Serial.printf("erase device: %s", address.toString().c_str());
...
void NimBLEScanResults::dump() {
    ...
    for (int i=0; i<getCount(); i++) {
        //Serial.printf("- %s", getDevice(i).toString().c_str());
    }
} // dump
