...
{
    //Serial.printf(">> NimBLERemoteCharacteristic()");
    ...
    //Serial.printf("<< NimBLERemoteCharacteristic(): %s", m_uuid.toString().c_str());
...
bool NimBLERemoteCharacteristic::retrieveDescriptors(const NimBLEUUID *uuid_filter) {
    //Serial.printf(">> retrieveDescriptors() for characteristic: %s", getUUID().toString().c_str());
    ...
    if (rc != 0) {
        //Serial.printf("ble_gattc_disc_all_chrs: rc=%d %s", rc, NimBLEUtils::returnCodeToString(rc));
        return false;
    }
    ...
    if(taskData.rc != 0) {
        //Serial.printf("ble_gattc_disc_all_chrs: startHandle:%d endHandle:%d taskData.rc=%d %s", m_handle, endHandle, taskData.rc, NimBLEUtils::returnCodeToString(0x0100+taskData.rc));
        return false;
    }

    return true;
    //Serial.printf("<< retrieveDescriptors(): Found %d descriptors.", m_descriptorVector.size());
} // retrieveDescriptors
...
NimBLERemoteDescriptor* NimBLERemoteCharacteristic::getDescriptor(const NimBLEUUID &uuid) {
    //Serial.printf(">> getDescriptor: uuid: %s", uuid.toString().c_str());

    for(auto &it: m_descriptorVector) {
        if(it->getUUID() == uuid) {
            //Serial.printf("<< getDescriptor: found the descriptor with uuid: %s", uuid.toString().c_str());
            return it;
        }
    }
    ...
    //Serial.printf("<< getDescriptor: Not found");
    return nullptr;
} // getDescriptor
...
std::vector<NimBLERemoteDescriptor*>* NimBLERemoteCharacteristic::getDescriptors(bool refresh) {
    if(refresh) {
        ...
        if (!retrieveDescriptors()) {
            //Serial.printf("Error: Failed to get descriptors");
        }
        else{
            //Serial.printf("Found %d descriptor(s)", m_descriptorVector.size());
        }
    }
...
std::string NimBLERemoteCharacteristic::readValue(time_t *timestamp) {
    //Serial.printf(">> readValue(): uuid: %s, handle: %d 0x%.2x",getUUID().toString().c_str(), getHandle(), getHandle());
    ...
    do {
        rc = ble_gattc_read_long(pClient->getConnId(), m_handle, 0,
                                 ...
                                 &taskData);
        if (rc != 0) {
            //Serial.printf("Error: Failed to read characteristic; rc=%d, %s",rc, NimBLEUtils::returnCodeToString(rc));
            return value;
        }
        ...
        switch(rc){
            ...
            case BLE_HS_ATT_ERR(BLE_ATT_ERR_ATTR_NOT_LONG):
                //Serial.printf("Attribute not long");
                rc = 0;
                break;
            ...
            /* Else falls through. */
            default:
                //Serial.printf("<< readValue rc=%d", rc);
                return value;
        }
    ...
    //Serial.printf("<< readValue length: %d rc=%d", value.length(), rc);
    return value;
} // readValue
                ...
                struct ble_gatt_attr *attr, void *arg)
{
    ...
    //Serial.printf("Read complete; status=%d conn_handle=%d", error->status, conn_handle);
    ...
    if(rc == 0) {
        if(attr) {
            if(((*strBuf).length() + attr->om->om_len) > BLE_ATT_ATTR_MAX_LEN) {
                ...
                //Serial.printf("Got %d bytes", attr->om->om_len);
...
bool NimBLERemoteCharacteristic::setNotify(uint16_t val, notify_callback notifyCallback, bool response) {
    //Serial.printf(">> setNotify(): %s, %02x", toString().c_str(), val);

    m_notifyCallback = notifyCallback;

    NimBLERemoteDescriptor* desc = getDescriptor(NimBLEUUID((uint16_t)0x2902));
    if(desc == nullptr) {
        //Serial.printf("<< setNotify(): Callback set, CCCD not found");
        return true;
    }

    //Serial.printf("<< setNotify()");
...
void NimBLERemoteCharacteristic::deleteDescriptors() {
    //Serial.printf(">> deleteDescriptors");
...
size_t NimBLERemoteCharacteristic::deleteDescriptor(const NimBLEUUID &uuid) {
    //Serial.printf(">> deleteDescriptor");
...
bool NimBLERemoteCharacteristic::writeValue(const uint8_t* data, size_t length, bool response) {
    ...
    //Serial.printf(">> writeValue(), length: %d", length);
    ...
    do {
        ...
        if (rc != 0) {
            //Serial.printf("Error: Failed to write characteristic; rc=%d", rc);
            return false;
        }
        ...
        switch(rc){
            ...
            case BLE_HS_EDONE:
                ...
                break;
            case BLE_HS_ATT_ERR(BLE_ATT_ERR_ATTR_NOT_LONG):
                //Serial.printf("Long write not supported by peer; Truncating length to %d", mtu);
                retryCount++;
                length = mtu;
                break;
            ...
            /* Else falls through. */
            default:
                //Serial.printf("<< writeValue, rc: %d", rc);
                return false;
        }
    } while(rc != 0 && retryCount--);
                ...
                struct ble_gatt_attr *attr, void *arg)
{
    ...
    //Serial.printf("Write complete; status=%d conn_handle=%d", error->status, conn_handle);
