...
int NimBLEDescriptor::handleGapEvent(uint16_t conn_handle, uint16_t attr_handle,
    ...
    //Serial.printf("Descriptor %s %s event", pDescriptor->getUUID().toString().c_str(),ctxt->op == BLE_GATT_ACCESS_OP_READ_DSC ? "Read" : "Write");
...
void NimBLEDescriptor::setHandle(uint16_t handle) {
    //Serial.printf(">> setHandle(0x%.2x): Setting descriptor handle to be 0x%.2x", handle, handle);
    m_handle = handle;
    //Serial.printf("<< setHandle()");
} // setHandle
...
void NimBLEDescriptor::setValue(const uint8_t* data, size_t length) {
    if (length > m_value.attr_max_len) {
        //Serial.printf("Size %d too large, must be no bigger than %d", length, m_value.attr_max_len);
        return;
    }
