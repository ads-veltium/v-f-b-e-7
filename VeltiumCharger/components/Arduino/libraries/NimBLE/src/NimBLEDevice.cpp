...
uint16_t                    NimBLEDevice::m_scanDuplicateSize = 10;
uint8_t                     NimBLEDevice::m_scanFilterMode = 2;
...
/* STATIC */ void NimBLEDevice::setPower(esp_power_level_t powerLevel, esp_ble_power_type_t powerType) {
    //Serial.printf(">> setPower: %d (type: %d)", powerLevel, powerType);
    esp_err_t errRc = esp_ble_tx_power_set(powerType, powerLevel);
    if (errRc != ESP_OK) {
        //Serial.printf("esp_ble_tx_power_set: rc=%d", errRc);
    }
    //Serial.printf("<< setPower");
} // setPower
...
/* STATIC*/ NimBLEAddress NimBLEDevice::getAddress() {
    ...
    if(BLE_HS_ENOADDR == ble_hs_id_copy_addr(BLE_ADDR_PUBLIC, addr.val, NULL)) {
        //Serial.printf("Public address not found, checking random");
...
/* STATIC */int NimBLEDevice::setMTU(uint16_t mtu) {
    //Serial.printf(">> setLocalMTU: %d", mtu);

    int rc =  ble_att_set_preferred_mtu(mtu);

    if (rc != 0) {
        //Serial.printf("Could not set local mtu value to: %d", mtu);
    }

    //Serial.printf("<< setLocalMTU");
    return rc;
} // setMTU
...
void NimBLEDevice::setScanDuplicateCacheSize(uint16_t cacheSize) {
    if(initialized) {
        //Serial.printf("Cannot change scan cache size while initialized");
        return;
    } else if(cacheSize > 1000 || cacheSize <10) {
        //Serial.printf("Invalid scan cache size; min=10 max=1000");
        return;
    }
...
void NimBLEDevice::setScanFilterMode(uint8_t mode) {
    if(initialized) {
        //Serial.printf("Cannot change scan duplicate type while initialized");
        return;
    } else if(mode > 2) {
        //Serial.printf("Invalid scan duplicate type");
        return;
    }
...
{
    //Serial.printf("NimBle host synced.");
...
/* STATIC */ void NimBLEDevice::deinit(bool clearAll) {
    ...
    if (ret == 0) {
        ...
        if (ret != ESP_OK) {
            //Serial.printf("esp_nimble_hci_and_controller_deinit() failed with error: %d", ret);
        }
...
/*STATIC*/ void NimBLEDevice::setSecurityAuth(bool bonding, bool mitm, bool sc) {
    //Serial.printf("Setting bonding: %d, mitm: %d, sc: %d",bonding,mitm,sc);
...
/* STATIC */int NimBLEDevice::startSecurity(uint16_t conn_id) {
  ...
  */
    ...
    if(rc != 0){
        //Serial.printf("ble_gap_security_initiate: rc=%d %s", rc, NimBLEUtils::returnCodeToString(rc));
    }
...
void NimBLEDevice::setCustomGapHandler(gap_event_handler handler) {
    ...
    if(rc == BLE_HS_EALREADY){
        //Serial.printf("Already listening to GAP events.");
    }
    else{
        assert(rc == 0);
    }
