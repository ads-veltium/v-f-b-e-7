...
{
    //Serial.printf(">> NimBLERemoteDescriptor()");
    ...
    //Serial.printf("<< NimBLERemoteDescriptor(): %s", m_uuid.toString().c_str());
}
...
std::string NimBLERemoteDescriptor::readValue() {
    //Serial.printf(">> Descriptor readValue: %s", toString().c_str());
    ...
    do {
        rc = ble_gattc_read_long(pClient->getConnId(), m_handle, 0,
                                 ...
                                 &taskData);
        if (rc != 0) {
            //Serial.printf("Error: Failed to read descriptor; rc=%d, %s",rc, NimBLEUtils::returnCodeToString(rc));
            return value;
        }
    ...
    //Serial.printf("<< Descriptor readValue(): length: %d rc=%d", value.length(), rc);
    return value;
} // readValue
...
bool NimBLERemoteDescriptor::writeValue(const uint8_t* data, size_t length, bool response) {
    ...
    //Serial.printf(">> Descriptor writeValue: %s", toString().c_str());
    ...
    do {
        ...
        if (rc != 0) {
            //Serial.printf("Error: Failed to write descriptor; rc=%d", rc);
            return false;
        }
    ...
    //Serial.printf("<< Descriptor writeValue, rc: %d",rc);
    return (rc == 0);
} // writeValue
