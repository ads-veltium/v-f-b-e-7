...
uint8_t NVS::init(const char *partition, const char *name, uint8_t debug=1)
{
    esp_err_t err;
    _debug = debug;
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::erase_key(const char *key)
{
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::create(const char *key, const char *value)
{
    esp_err_t err;
    size_t sz;
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::create(const char *key, int32_t value)
{
    esp_err_t err;
    int32_t bff;
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::create(const char *key, uint8_t value)
{
    esp_err_t err;
    uint8_t bff;

    err = nvs_get_u8(_handler, key, &bff);
    if (err == ESP_ERR_NVS_NOT_FOUND)
    {
        err = nvs_set_u8(_handler, key, value);
        if (err != ESP_OK)
        {
            ESP_LOGE(tag, "Fail to create key ['%s'] [0x%x]", key, err);
            return 0;
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::write(const char *key, const char *value)
{
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::write(const char *key, int32_t value)
{
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::write(const char *key, uint8_t value){
    esp_err_t err = nvs_set_u8(_handler, key, value);
    if (err != ESP_OK)
    {
        return 0;
    }
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::read(const char *key, char *dst, uint16_t size)
{
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::read(const char *key, int32_t *dst)
{
 ...
 * @return 1: Sucess.
 */
uint8_t NVS::read(const char *key, uint8_t *dst)
{
    esp_err_t err = nvs_get_u8(_handler, key, dst);
    if (err != ESP_OK)
    {
        ESP_LOGE(tag, "Fail to read key ['%s'] [0x%x]", key, err);
        return 0;
