...
#ifdef USE_COMS
		else if((Params.Tipo_Sensor || ChargingGroup.Params.CDP >> 4) && ContadorExt.ConexionPerdida){
			Kit(NARANJA_OSCURO);
			Delay= 85;
		}
		//Buscando Medidor
		else if((Params.Tipo_Sensor || ChargingGroup.Params.CDP >> 4)  && !ContadorExt.MeidorConectado){
			//Buscando Gateway
