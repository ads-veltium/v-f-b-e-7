...
int CyBtldr_RunAction(CyBtldr_Action action, HardwareSerialMOD *comm, CyBtldr_ProgressUpdate *update, const char *file, const char *partition)
{
    g_abort = 0;
    uint32_t lineLen;
    ...
    if (CYRET_SUCCESS == err) {
        ...
        if (CYRET_SUCCESS == err) {
            ...
            if (err == CYRET_SUCCESS && (PROGRAM == action || VERIFY == action)) {
                ...
                CyBtldr_EndBootloadOperation(comm);
...
int CyBtldr_Program(const char *file, const char *partition, HardwareSerialMOD *comm, CyBtldr_ProgressUpdate *update)
{
    return CyBtldr_RunAction(PROGRAM, comm, update, file, partition);
}

int CyBtldr_Erase(const char *file, const char *partition, HardwareSerialMOD *comm, CyBtldr_ProgressUpdate *update)
{
    return CyBtldr_RunAction(ERASE, comm, update, file, partition);
}

int CyBtldr_Verify(const char *file, const char *partition, HardwareSerialMOD *comm, CyBtldr_ProgressUpdate *update)
{
    return CyBtldr_RunAction(VERIFY, comm, update, file, partition);
}
