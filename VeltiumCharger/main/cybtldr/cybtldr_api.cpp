...
int CyBtldr_StartBootloadOperation(HardwareSerialMOD* ReceivedChannel, unsigned long expSiId, unsigned char expSiRev, unsigned long* blVer)
{
...
int CyBtldr_EndBootloadOperation(HardwareSerialMOD* ReceivedChannel)
{
    ...
    if (CYRET_SUCCESS == err)
    {
        err = channel->write(inBuf, inSize);
...
int CyBtldr_ProgramRow(unsigned char arrayID, unsigned short rowNum, unsigned char* buf, unsigned short size)
{
    ...
    while ((CYRET_SUCCESS == err) && ((size - offset + TRANSFER_HEADER_SIZE) > 128))
    {
        subBufSize = (unsigned short)(128 - TRANSFER_HEADER_SIZE);
